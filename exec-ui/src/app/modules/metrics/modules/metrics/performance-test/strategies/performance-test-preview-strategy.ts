...
export class PerformanceTestPreviewStrategy extends PreviewStrategyBase {
  ...
  public parse(model: MetricDetail): MetricPreviewModel {
    ...
    private calculateSecondaryMetric(model: MetricSummary) {
        ...
        //if (sums.get('tps')) {
       //     result.push({name: 'TPS', value: Math.round(sums.get('tps').toLocaleString())});
        //}

        //if (sums.get('response time')) {
         //   result.push({name: 'response time', value: sums.get('response time').toLocaleString()});
       // }
